...
function normalizeItem(x){
  ...
  const definition = safeStr(x.definition || x.def || x.explain || x.explanation);
...
function rowHtml(item){
  ...
  const definition = item.definition
    ? `<div class="defBox">${escapeHtml(item.definition)}</div>`
    : `<span class="muted">—</span>`;
...
function openEdit(id){
  ...
  $("edDefinition").value = item.definition || "";
...
async function saveEdit(){
  ...
  item.definition = safeStr($("edDefinition").value).trim();
  ...
  if (btnReview) {
    btnReview.addEventListener("click", () => {
      const url = (typeof chrome !== "undefined" && chrome.runtime && chrome.runtime.getURL)
        ? chrome.runtime.getURL("review_envi.html")
        : "review_envi.html";
